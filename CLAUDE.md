...
- **Zahlenblock-Anzeige** (`#np-value`) traegt BEWUSST kein negatives `letter-spacing` und eine feste `min-width`.
  Der Platzhalter fuer ein leeres Feld ist ein Gedankenstrich „–", also selbst eine waagerechte Linie. Schrumpfte
  die Box beim Wechsel auf die Ziffer, zeichnete iOS den Bereich unvollstaendig neu und der Strich blieb quer
  durch die Zahl stehen (in Chrome nicht reproduzierbar).
...
- **Detailansicht einer vergangenen Einheit** (`showHistDetail(i, highlightExId)` → `#modal-hist-detail`) ist ein ZEITSTRAHL
  ...
  Sätze ohne Gewicht zeigen nur die Wiederholungen. Die Hervorhebung sitzt auf `.hd-step-body`, NICHT auf `.hd-step` —
