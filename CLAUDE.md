...
- `getLastExData(exId)` = Sätze der letzten ABGESCHLOSSENEN Einheit. `buildSetsForExercise` seedet den Workout-Start.
...
- **Trainingstag-Namen** = feste pinke Pille `.pd-name` (#FCE7F3 / #9D174D) via Helper `pd(name)`.
...
- **Wochenplan-Strips:** `.ppv-*` (Dashboard-Karte, `buildPlanCard(p, onTap, hideToday, hideStatus)`) + `.wp-col` (Workouts-Strip, `buildWpCol`). Erledigte Trainings dieser Woche → grünes ✓ (`.wp-col-done-check` / `.ppv-done-check`).
